...
class TableStore(filename:String, twoLineHeader:Boolean = false, inTableSubdir:Boolean = true) {
  ...
  def getRowByUID(uid:String):TableRow = {
    ...
    if (tableIdx == -1) {
      ...
      return new TableRow(new Table("", this), Array.empty[String], Array.empty[Array[Array[Int]]], Array.empty[Array[Array[Int]]], Array.empty[Array[Array[Int]]], Array.empty[Array[Array[Int]]], Array.empty[Array[Array[Int]]])     // Note: Not ideal -- creates a new empty table reference for each UID that isn't found.
    }
