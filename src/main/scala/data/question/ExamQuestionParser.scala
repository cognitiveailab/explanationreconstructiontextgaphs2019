...
object ExamQuestionParser {
      ...
      */
      ...
      if (isMultipleChoice) {
        ...
        val rq = new ExamQuestion(qID, originalID, totalPoints, answerKey, isMultipleChoice,
          hasDiagram, examName, schoolGrade, year, questionType, topics, fold = "", goldJustification, questionText, mcAnswers)
        questions.append(rq)

      } else {
        val rq = new ExamQuestion(qID, originalID, totalPoints, answerKey, isMultipleChoice,
          hasDiagram, examName, schoolGrade, year, questionType, topics, fold = "", goldJustification, question, Array.empty[String])
        questions.append(rq)
      }
  ...
  def toMCQuestion(in:Array[MCQuestionRaw], fullAnnotation:Boolean = true):Array[MCQuestion] = {
    ...
    for (i <- 0 until in.size) {
      ...
      out(i) = new MCQuestion(question.text, qAnnotation, question.qType, question.justification, jAnnotation, question.grade, topics.toArray, fold = "", question.questionID, question.correctAnswer, choices)
    }
