...
object ExamQuestionParserDynamic {
  ...
  val LABEL_GRADEALT          =   "grade".toUpperCase
  ...
  val LABEL_TOPICALT                =   "consensus".toUpperCase
  ...
  val LABEL_FOLD                    =   "fold".toUpperCase
  ...
  def getColumnMapping(headerFields:List[String]):(scala.collection.mutable.Map[String, Int], String) = {
    ...
    for (fieldIdx <- 0 until headerFields.size) {
      ...
      headerFieldText match {
        ...
        case LABEL_GRADEALT                 => out(LABEL_GRADE) = fieldIdx
        ...
        case LABEL_TOPICALT                 => out(LABEL_TOPIC) = fieldIdx
        ...
        case LABEL_FOLD                     => out(LABEL_FOLD) = fieldIdx
  ...
  // Convert from letter indicies of answer candidates (A, B, C, D, E) to numeric Int indicies (0, 1, 2, 3, 5)
  ...
  def load(filename:String, tsvMode:Boolean = false):(Array[ExamQuestion], String) = {
    ...
    for (fields <- lines) {
      ...
      breakable {
        ...
        val fold                      = safeGetField(LABEL_FOLD, headerMap, fields).getOrElse("")
        ...
        if (isMultipleChoice) {
          ...
          val rq = new ExamQuestion(qID, originalID, totalPoints, answerKey, isMultipleChoice,
            hasDiagram, examName, schoolGrade, year, questionType, topics, fold, goldExplanation, questionText, mcAnswers,
          ...
          val rq = new ExamQuestion(qID, originalID, totalPoints, answerKey, isMultipleChoice,
            hasDiagram, examName, schoolGrade, year, questionType, topics, fold, goldExplanation, question, Array.empty[String],
            flagsCounter, explanationAnnoatators,
            questionSimplified, questionSimplifiedConc, questionFocusPrimary, questionFocusSecondary, answerFocus)
          questions.append(rq)
        }
  ...
  def toMCQuestion(in:Array[ExamQuestion], fullAnnotation:Boolean = true, noAnnotation:Boolean = false, annotateExplanations:Boolean = true):Array[MCQuestion] = {
    ...
    for (i <- 0 until in.size) {
      ...
      out(i) = new MCQuestion(question.text, qAnnotation, question.questionType, question.explanation, jAnnotation, question.schoolGrade, topics.toArray, question.fold, question.qID,
